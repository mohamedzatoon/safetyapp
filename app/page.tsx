...
function NavItem({
  ...
  return (
    ...
    >
      ...
      <span className={centered ? 'text-[10px] font-medium' : 'text-[11px]'}>{label}</span>
    </button>
  );
}
    ...
    <div className="absolute bottom-4 left-4 right-4 z-20">
      <div className="flex items-end justify-between rounded-[1.75rem] border border-white/10 bg-black/55 px-6 py-4 backdrop-blur-xl shadow-2xl">
        <div className="relative">
          <NavItem icon={FileText} label="Archive" active={active === 'archive'} onClick={onArchive} />
          {archiveCount > 0 && (
        ...
        <NavItem icon={Upload} label="Upload" active={active === 'upload'} onClick={onUpload} />
      </div>
    </div>
  );
}
...
function getSeverityClasses(severity: Detection['severity']) {
  if (severity === 'Critical') return 'border-red-400/80 bg-red-500/10 text-red-200';
  if (severity === 'High') return 'border-amber-400/80 bg-amber-500/10 text-amber-200';
  return 'border-cyan-400/80 bg-cyan-500/10 text-cyan-200';
}

function getSeverityBadgeClasses(severity: Detection['severity']) {
  if (severity === 'Critical') return 'bg-red-100 text-red-700 hover:bg-red-100';
  if (severity === 'High') return 'bg-amber-100 text-amber-700 hover:bg-amber-100';
  return 'bg-cyan-100 text-cyan-700 hover:bg-cyan-100';
}

export default function Page() {
  const [screen, setScreen] = useState<'capture' | 'analysis' | 'archive' | 'results'>('capture');
  const [progress, setProgress] = useState(0);
  const [image, setImage] = useState<string | null>(null);
  const [sourceType, setSourceType] = useState<'upload' | 'camera'>('upload');
  ...
  const highestConfidence = useMemo(() => {
    ...
    return `${Math.round(Math.max(...detections.map((item) => (item.confidence || 0) * 100)))}%`;
  }, [detections]);
  ...
  const handleImageFile = (file: File | undefined, type: 'upload' | 'camera') => {
    if (!file) return;
    setSourceType(type);
    setErrorMessage('');
  ...
  const simulateProgress = () => {
    setProgress(0);
    [12, 26, 41, 58, 74, 89].forEach((value, index) => {
      setTimeout(() => setProgress(value), (index + 1) * 260);
    });
  };

  const finishAnalysis = (normalized: Detection[], textSummary: string, sourceLabel: string) => {
    ...
    setReportReady(true);
    if (image) {
      saveToArchive({ imageSrc: image, detectionList: normalized, textSummary, sourceLabel });
    }
  };
  ...
  const startAnalysis = async () => {
    ...
    try {
      const response = await fetch('/api/analyze', {
        ...
        body: JSON.stringify({ image, siteName, sourceType }),
      });

      if (!response.ok) throw new Error(`API returned ${response.status}`);
      const data = await response.json();
      const normalized = Array.isArray(data?.detections) ? data.detections.map(normalizeDetection) : [];
      if (!normalized.length) throw new Error('No detections returned from backend');
      finishAnalysis(normalized, data?.summary || 'Live analysis completed successfully.', 'Live backend');
    } catch {
      setErrorMessage('Live analysis endpoint is not connected yet. Demo output is shown instead so the flow remains testable.');
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-8 flex items-center justify-between rounded-[1.75rem] border border-white/10 bg-white/5 px-6 py-5 backdrop-blur-xl">
          <div>
            ...
            <h1 className="mt-1 text-2xl font-semibold text-white">Simple safety inspection workflow</h1>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setAnalysisMode('demo')}
              className={`rounded-full px-3 py-2 text-sm transition ${analysisMode === 'demo' ? 'bg-white text-slate-950' : 'bg-white/5 text-white/70 hover:bg-white/10'}`}
            >
              Demo
            </button>
            <button
              onClick={() => setAnalysisMode('live')}
              className={`rounded-full px-3 py-2 text-sm transition ${analysisMode === 'live' ? 'bg-white text-slate-950' : 'bg-white/5 text-white/70 hover:bg-white/10'}`}
            >
              Live
            </button>
          </div>
        ...
        <div className="grid gap-8 xl:grid-cols-[430px_1fr]">
          <PhoneShell>
            <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="relative z-10 p-6 pt-12">
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="text-xs uppercase tracking-[0.25em] text-white/40">Inspection Capture</div>
                        <h2 className="mt-2 text-3xl font-semibold">Field Evidence</h2>
                      </div>
                      {image && (
                        <button
                          onClick={resetAll}
                    ...
                    <div className="mt-6 overflow-hidden rounded-[2rem] border border-white/10 bg-black/30">
                      {image ? (
                        <div className="relative aspect-[9/13] w-full overflow-hidden bg-black">
                          <img src={image} alt="inspection preview" className="h-full w-full object-cover" />
                          <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
                          {detections.map((item) =>
                            item.bbox ? (
                              <div
                                key={item.id}
                                className={`absolute rounded-xl border-2 ${item.severity === 'Critical' ? 'border-red-400' : 'border-amber-400'}`}
                                style={{
                                  left: `${item.bbox.x}%`,
                                  top: `${item.bbox.y}%`,
                                  width: `${item.bbox.w}%`,
                              ...
                              >
                                <div className={`-mt-7 inline-flex rounded-full border px-2 py-1 text-[10px] font-semibold backdrop-blur ${getSeverityClasses(item.severity)}`}>
                                  {item.title}
                                </div>
                              </div>
                            ) : null
                          ...
                          <div className="absolute left-4 right-4 top-4 flex items-center justify-between">
                            <div className="rounded-full bg-black/45 px-3 py-1 text-xs text-white/80 backdrop-blur">
                              {sourceType === 'camera' ? 'Camera image' : 'Uploaded image'}
                            </div>
                            <div className="rounded-full bg-black/45 px-3 py-1 text-xs text-white/80 backdrop-blur">
                              {activeModeLabel}
                            </div>
                        ...
                        <div className="relative aspect-[9/13] w-full overflow-hidden bg-[linear-gradient(180deg,#10233b_0%,#0d1b2f_22%,#223850_22%,#314963_42%,#17212f_42%,#0c1420_100%)]">
                          ...
                          <div className="absolute bottom-5 left-5 right-5 rounded-[1.5rem] border border-white/10 bg-black/40 p-5 backdrop-blur-md">
                            ...
                            <h3 className="mt-3 text-xl font-semibold text-white">Upload or capture a site photo</h3>
                            <p className="mt-2 text-sm leading-6 text-white/60">Keep it simple. Add an image, then run analysis.</p>
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    <input
                      ...
                      type="file"
                      accept="image/*"
                      onChange={(e) => handleImageFile(e.target.files?.[0], 'upload')}
                      className="hidden"
                    />
                    <input
                      ref={cameraInputRef}
                      ...
                      accept="image/*"
                      capture="environment"
                      onChange={(e) => handleImageFile(e.target.files?.[0], 'camera')}
                      className="hidden"
                    />
                  </div>
                </motion.div>
                ...
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="text-center">
                      ...
                      <h2 className="mt-6 text-3xl font-semibold">Analyzing Safety Scene</h2>
                      <p className="mt-3 text-sm leading-6 text-white/60">
                        Running visual inspection logic, code correlation, and report structuring.
                      </p>
                    </div>
                    ...
                    <div className="mt-6 space-y-3">
                      {stages.map((stage, index) => (
                        <div key={stage} className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 px-4 py-4">
                          <div className="flex items-center gap-3">
                            <div className={`rounded-full p-2 ${progress > index * 25 ? 'bg-emerald-400/15 text-emerald-300' : 'bg-white/5 text-white/40'}`}>
                              {progress > index * 25 ? <CheckCircle2 className="h-4 w-4" /> : <ScanSearch className="h-4 w-4" />}
                            </div>
              ...
              {screen === 'archive' && (
                <motion.div key="archive" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-5 pt-12 text-white">
                  <div className="flex items-center justify-between">
                  ...
                  <div className="mt-5 space-y-3 max-h-[620px] overflow-auto pr-1">
                    ...
                    {archive.map((item) => (
                      <div key={item.id} className="rounded-[1.5rem] border border-white/10 bg-white/5 p-3 backdrop-blur-xl">
                        <img src={item.image} className="mb-3 h-36 w-full rounded-[1rem] object-cover" alt="archive item" />
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <div className="text-sm text-white/80">{item.date}</div>
                            <div className="mt-1 text-xs text-cyan-300">{item.source}</div>
                          </div>
                          <Badge className="border-0 bg-white/10 text-white/80 hover:bg-white/10">
                            {item.detections?.length || 0} issues
                          </Badge>
                        </div>
                        <div className="mt-2 text-xs leading-5 text-white/55">{item.summary}</div>
                      </div>
                    ))}
                  </div>
                </motion.div>
              ...
              {screen === 'results' && (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="rounded-full bg-emerald-500/15 px-3 py-1 text-xs font-medium text-emerald-300">Report Ready</div>
                  </div>
                  ...
                  <div className="mt-5 rounded-[1.8rem] border border-white/10 bg-white/5 p-5 shadow-[0_20px_50px_rgba(15,23,42,0.08)] backdrop-blur-xl">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <div className="text-xs uppercase tracking-[0.22em] text-white/40">Inspection Summary</div>
                        <h2 className="mt-2 text-3xl font-semibold">Violation Report</h2>
                        <p className="mt-2 text-sm leading-6 text-white/60">{summary || 'Structured output based on the selected visual evidence.'}</p>
                      </div>
                    ...
                    <div className="mt-5 grid grid-cols-4 gap-3">
                      <div className="rounded-2xl bg-black/20 p-3">
                        ...
                        <div className="mt-1 text-xl font-semibold">{detections.length}</div>
                      </div>
                      <div className="rounded-2xl bg-black/20 p-3">
                      ...
                      <div className="rounded-2xl bg-black/20 p-3">
                        <div className="text-xs text-white/40">Top confidence</div>
                        <div className="mt-1 text-sm font-semibold">{highestConfidence}</div>
                      </div>
                      <div className="rounded-2xl bg-black/20 p-3">
                        <div className="text-xs text-white/40">Source</div>
                        <div className="mt-1 text-sm font-semibold">{analysisSource}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 space-y-3">
                    {detections.map((item) => {
                      ...
                      return (
                        <div key={item.id} className="rounded-[1.5rem] border border-white/10 bg-white/5 p-4 shadow-[0_12px_30px_rgba(15,23,42,0.05)] backdrop-blur-xl">
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex gap-3">
                              ...
                              <div>
                                ...
                                <div className="mt-1 text-sm text-cyan-300">{item.code}</div>
                            ...
                            <Badge className={`rounded-full border-0 px-3 py-1 ${getSeverityBadgeClasses(item.severity)}`}>
                              {item.severity}
                            </Badge>
                          </div>
                        </div>
                  ...
                  <div className="mt-5 grid grid-cols-2 gap-3">
                    <Button onClick={downloadReport} className="h-14 rounded-2xl border-0 bg-slate-900 text-base font-semibold text-white shadow-lg hover:bg-slate-800">
                      <Download className="mr-2 h-5 w-5" />
                      Download
                    </Button>
                    <Button onClick={resetAll} variant="outline" className="h-14 rounded-2xl border-white/10 bg-white/5 text-base font-semibold text-white hover:bg-white/10">
                      New Scan
                    </Button>
                  </div>
                </motion.div>
          ...
          <div className="space-y-4">
            <Card className="rounded-[1.75rem] border border-white/10 bg-white/5 text-white backdrop-blur-xl">
              <CardContent className="p-5">
                ...
                <div className="mt-2 text-lg font-semibold">Archive, upload, capture, analyze</div>
                <p className="mt-2 text-sm leading-6 text-white/60">
                  This version is cleaner, more stable, and easier to send for demo testing. Archive now saves correctly and persists during the session.
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-[1.75rem] border border-white/10 bg-white/5 text-white backdrop-blur-xl">
              <CardContent className="p-5">
                ...
                <div className="mt-2 text-lg font-semibold">{analysisSource}</div>
                <p className="mt-2 text-sm leading-6 text-white/60">
                  Demo mode is the safest for sharing right now. Live mode only works when a real backend endpoint is connected.
                </p>
              </CardContent>
            </Card>
          </div>
